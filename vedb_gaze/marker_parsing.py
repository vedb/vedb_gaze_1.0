...
def _bimodality_check(
    data, n_stds_separate=2.5,
):
...
def remove_brief_detections(
    markers, all_timestamps, duration_threshold=0.6, is_verbose=False
):
    ...
    output = dict(
        (key, value[~duplicates][keep_index]) for key, value in markers.items()
    )
    return output
...
def remove_small_detections(
    ...
    if "size" in markers:
        ...
        # TODO: figure out why this is in here. It filters out all the markers in the sample data set
        # mksz_xy[:, 0] *= image_aspect_ratio
    else:
        raise ValueError(
            "Must have 'size' or 'norm_pos_full_checkerboard' parameter in marker dictionary"
        )
    mksz = mksz_xy.mean(1)
    ...
    if return_rejects:
        filtered_markers = utils.dictlist_to_arraydict(utils.filter_list(lst, ~keepers))
    else:
        filtered_markers = utils.dictlist_to_arraydict(utils.filter_list(lst, keepers))
    if is_verbose:
    ...
    *data
    ...
    if is_verbose:
        ...
        for x, dur in zip(break_indices[1:-1], epoch_durations):
            print(
                "@ %d min, %.1f s : %.1f s long"
                % ((timestamps[x] - t0) // 60, (timestamps[x] - t0) % 60, dur)
            )
        ...
        print(
            "%d epochs meet duration limit (%d-%s seconds)"
            % (len(output), min_epoch_length, max_length_str)
        )
    return output


def marker_cluster_stat(
    markers, fn=np.nanmedian, clusters=None, field="norm_pos", return_all_fields=True
):
    ...
    if clusters is None:
        if "marker_cluster_index" in markers:
            ...
            raise ValueError(
                (
                    "Please provide `clusters` kwarg if input dict \n"
                    "does not have `'marker_cluster_index'` field"
                )
            )

    tmp_field_value = np.array(
        [fn(markers[field][clusters == ci], axis=0) for ci in np.unique(clusters)]
    )
    if return_all_fields:
        mk_index = [
            np.argmin(np.abs(np.array(markers[field]) - mk_pos).mean(1))
...
def cluster_marker_points(
    markers,
    pupil_left=None,
    pupil_right=None,
    cluster_by=("marker:timestamp", "marker:norm_pos",),
    ...
    to_kill = np.any(np.isnan(to_cluster), axis=1) | np.any(to_cluster == 0, axis=1)
    if np.any(to_kill):
    ...
    group_durations = marker_cluster_stat(
        markers, field="timestamp", fn=np.ptp, return_all_fields=False
    )
    if is_verbose:
    ...
    if max_marker_movement is not None:
        group_jitter = marker_cluster_stat(
            markers, field="norm_pos", fn=np.ptp, return_all_fields=False
        )
        ...
        if is_verbose:
            print(
                "%d groups after marker jitter filtering" % (keep_clusters_binary.sum())
            )
    if max_pupil_movement is not None:
        pupil_jitter_left = marker_cluster_stat(
            pupil_left, field="norm_pos", fn=np.ptp, return_all_fields=False
        )
        keep_clusters_binary &= np.all(pupil_jitter_left < max_pupil_movement, axis=1)
        pupil_jitter_right = marker_cluster_stat(
            pupil_right, field="norm_pos", fn=np.ptp, return_all_fields=False
        )
        keep_clusters_binary &= np.all(pupil_jitter_right < max_pupil_movement, axis=1)
        if is_verbose:
            print(
                "%d groups after pupil jitter filtering" % (keep_clusters_binary.sum())
            )
    # Keep clusters that have a standard deviation lower than a set threshold
    if max_cluster_std is not None:
        marker_cluster_stds = marker_cluster_stat(
            markers, field="norm_pos", fn=np.std, return_all_fields=False
        )
        marker_cluster_stds[:, 0] *= aspect_ratio
        keep_clusters_binary &= marker_cluster_stds.mean(1) < max_cluster_std
        if is_verbose:
            print("%d groups after marker std filtering" % (keep_clusters_binary.sum()))
...
def find_epochs(
    ...
    if do_duration_pre_check:
        marker = remove_brief_detections(
            marker, all_timestamps, duration_threshold=duration_threshold
        )
    marker = remove_small_detections(
        marker,
    ...
    if pupil_left is not None:
        raise NotImplementedError(
            (
                "If you wish to use pupil detections to help find clusters\n"
        ...
        pupil_left_match = utils.match_time_points(marker, pupil_left, window=window)
    ...
    if pupil_right is not None:
        ...
        pupil_right_match = utils.match_time_points(marker, pupil_right, window=window)
        to_split.append(pupil_right_match)

    # Split into multiple epochs of calibration, validation as needed
    epochs = split_timecourse(
        *to_split,
    ...
    if do_cluster_clean:
        ...
        for i, ce in enumerate(epochs):
            ...
            epoch_grpclean = cluster_marker_points(
                *ce,
