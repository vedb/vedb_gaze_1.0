...
def colormap_2d(
    ...
    """ Taken from the old visualization code
    https://github.com/vedb/vedb-gaze/blob/main/vedb_gaze/visualization.py
...
def calibrate_2d_polynomial(
    ...
    err_dist, err_mean, err_rms = calibrate_2d.fit_error_screen(
        err_x, err_y, screen_size
    )
    if max_stds_for_outliers is not None:
        print("Errors:")
        print(err_dist.tolist())
        inliers = err_dist < np.median(err_dist) + max_stds_for_outliers * np.std(
            err_dist
        )
    ...
    if np.any(err_ok):  # did not disregard all points..
        ...
        logger.error("The data cannot be represented by the model in a meaningful way.")
        return (
            map_fn,
            inliers,
            ([p.tolist() for p in cx], [p.tolist() for p in cy], model_n),
...
def calibrate_2d_binocular(
    cal_pt_cloud_binocular, cal_pt_cloud0, cal_pt_cloud1, frame_size
):
...
def _fit_rbf_cv(
    ...
    def LeaveOneOutXval(smoothness, method):
        ...
        for i in range(len(marker_pos)):
            ...
            estimates[i, :] = [
                horizontal(pupil_pos[i, 0], pupil_pos[i, 1]),
                vertical(pupil_pos[i, 0], pupil_pos[i, 1]),
            ]
    ...
    #    SearchAndFit()
    return (
        horizontalInterpolater,
        verticalInterpolater,
        bestMethod,
        bestSmoothness,
        bestError,
    )
...
def _fit_tps_gaze(
    calibration_markers, pupil, lambd=0.01, min_calibration_threshold=0.6,
):
...
def _fit_tps_gaze_cv(
    calibration_markers,
    pupil,
    lambd_list=DEFAULT_LAMBDA_LIST,
    min_calibration_threshold=0.6,
    max_stds_for_outliers=None,
    recursive_outlier_cut=False,
):
    """Cross-validated thin-plate spline fit for calibration"""
    keep = pupil["confidence"] > min_calibration_threshold
    mk = utils.filter_arraydict(calibration_markers, keep)
    pup = utils.filter_arraydict(pupil, keep)
    x, y = mk["norm_pos"].T
    ...
    for ia, lambd in enumerate(lambd_list):
        ...
        for i in range(len(x)):
            ...
            preds[i, 0] = tps.TPS.z(np.vstack([px[i], py[i]]).T, data_x[fit], theta_x)
            preds[i, 1] = tps.TPS.z(np.vstack([px[i], py[i]]).T, data_y[fit], theta_y)
    ...
    if max_stds_for_outliers is not None:
        ...
        if np.any(outliers):
            ...
            cal_cut = utils.filter_arraydict(mk, ~outliers)
            print(len(cal_cut["norm_pos"]))
            pupil_cut = utils.filter_arraydict(pup, ~outliers)
            print("Refitting...")
            if recursive_outlier_cut:
                mx_std = max_stds_for_outliers
            else:
                mx_std = None
            return _fit_tps_gaze_cv(
                cal_cut,
                pupil_cut,
...
def get_point_grid(
    ...
    tt_dense_h = np.linspace(st_horizontal, fin_horizontal, n_points_horizontal)
...
class Calibration(object):
    def __init__(
        ...
        **params,
        ...
        """
        calibration_list = utils.arraydict_to_dictlist(calibration_arrays)
        ...
        if "tag" in self.params:
            fpath = BASE_DIR / "config" / ("calibration_%s.yaml" % self.params["tag"])
            with open(fpath, "r") as fid:
                cal_params = yaml.safe_load(fid)
        ...
        if isinstance(pupil_arrays, (list, tuple)) and len(pupil_arrays) == 2:
            ...
            if self.mapping_computed:
                ...
                self.pupil_left = utils.match_time_points(
                    calibration_arrays, left_pupil,
                )
                self.pupil_right = utils.match_time_points(
                    calibration_arrays, right_pupil,
                )
                self.pupil_arrays = [self.pupil_left, self.pupil_right]
            self.pupil_list = utils.arraydict_to_dictlist(left_pupil)
            self.pupil_list.extend(utils.arraydict_to_dictlist(right_pupil))
            ...
            if not self.mapping_computed:
                self.pupil_arrays = utils.match_time_points(
                    calibration_arrays, pupil_arrays
                )
            self.pupil_list = utils.arraydict_to_dictlist(pupil_arrays)
    ...
    def _get_map_params(self):
        ...
        if self.cluster_reduce_fn is None:
            ...
            # confidence first:
            fn = utils.get_function(self.cluster_reduce_fn)
            keep = self.pupil_arrays["confidence"] > self.min_calibration_confidence
            marker_input_ = utils.filter_arraydict(self.calibration_arrays, keep)
            marker_input = marker_cluster_stat(
                marker_input_,
                fn=fn,
                field="norm_pos",
            ...
            pupil_input_ = utils.filter_arraydict(self.pupil_arrays, keep)
            pupil_input = {}
            pupil_input["timestamp"] = marker_input["timestamp"]
            pupil_input["norm_pos"] = marker_cluster_stat(
                pupil_input_,
                field="norm_pos",
                fn=fn,
            ...
            pupil_input["confidence"] = marker_cluster_stat(
                pupil_input_,
                field="confidence",
                fn=fn,
                clusters=marker_input_["marker_cluster_index"],
        ...
        if self.calibration_type == "monocular_pl":
            ...
            if "max_stds_for_outliers" in kws:
                _, outliers = _fit_tps_gaze_cv(
                    marker_input,
                    pupil_input,
                    min_calibration_threshold=self.min_calibration_confidence,
                    **kws,
                )
                marker_input = utils.filter_arraydict(marker_input, ~outliers)
            marker_list_input = utils.arraydict_to_dictlist(marker_input)
            is_binocular, matched_data = parse_plab_data(
                self.pupil_list,
                marker_list_input,
                mode="2d",
            ...
            if self.calibration_type == "monocular_tps":
                ...
                self.map_params, outliers = _fit_tps_gaze_cv(
                    marker_input,
                    pupil_input,
                    min_calibration_threshold=self.min_calibration_confidence,
                    **kws,
    ...
    def _get_mapper(self):
        if self.calibration_type == "monocular_pl":
            self._mapper = calibrate_2d.make_map_function(*self.map_params)
        elif self.calibration_type == "binocular_pl":
    ...
    def map(self, pupil_arrays, return_type="arraydict"):
        ...
        if self.calibration_type == "binocular_pl":
            ...
            # Left
            pupil_list = utils.arraydict_to_dictlist(pupil_arrays[0])
            # Right
            pupil_list.extend(utils.arraydict_to_dictlist(pupil_arrays[1]))
            ...
            gaze_array = utils.dictlist_to_arraydict(gaze)
            ...
            gaze_array = dict(timestamp=pupil_arrays["timestamp"], norm_pos=gaze)
        ...
        if return_type == "arraydict":
            ...
            return utils.arraydict_to_dictlist(gaze_array)
    ...
    def show_calibration(
        ...
        if self.calibration_type == "binocular_pl":
            ...
            if (grid_pts_array_l is None) or (grid_pts_array_r is None):
                ...
                gaze["left"] = utils.dictlist_to_arraydict(gaze["left"])
                gaze["right"] = [
                    g for g in gaze_binocular if g["base_data"][0]["id"] == 1
                ]
                gaze["right"] = utils.dictlist_to_arraydict(gaze["right"])
                grid_array = gaze[eye]["norm_pos"]
        elif self.calibration_type in (
            "monocular_pl",
            "monocular_tps",
            "monocular_tps_cv",
        ):
        ...
        if ax_eye is not None:
            ...
            if show_data:
                ax_eye.scatter(
                    *self.pupil_arrays["norm_pos"][ci].T,
                    s=self.pupil_arrays["confidence"][ci] * data_scale,
    ...
    def _get_grid_points(
        ...
        eye=None,
        sc=0.0,
        n_horizontal_lines=10,
        n_vertical_lines=None,
        n_points=40,
        return_type="dictlist",
        min_possible_eye_points=10,
    ):
        ...
        if return_type == "array":
            ...
            grid_pts_list = utils.arraydict_to_dictlist(grid_pts)
