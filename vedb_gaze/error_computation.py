...
def compute_error(
    ...
    if cluster_reduce_fn is not None:
        cluster_reduce_fn = utils.get_function(cluster_reduce_fn)
    ...
    if len(gaze["timestamp"]) != len(marker["timestamp"]):
        ...
        gaze_matched = utils.match_time_points(marker, gaze)
    ...
    if cluster_reduce_fn is not None:
        if not "marker_cluster_index" in marker:
            raise ValueError(
                "No clusters detected, can't perform cluster reduction and cross validation of lambda parameter"
            )
        else:
            clusters = marker["marker_cluster_index"][gz_ci]
            marker_pos = marker_cluster_stat(
                dict(marker_pos=marker_pos),
                fn=cluster_reduce_fn,
                field="marker_pos",
                return_all_fields=False,
                clusters=clusters,
            )
            gaze_pos = marker_cluster_stat(
                dict(gaze_pos=gaze_pos),
                fn=cluster_reduce_fn,
                field="gaze_pos",
                return_all_fields=False,
                clusters=clusters,
    ...
    if not isinstance(vertical_horizontal_smooth_error_resolution, (list, tuple)):
        hres, vres = (
            np.array(image_resolution) * vertical_horizontal_smooth_error_resolution
        ).astype(np.int)
    else:
    ...
    if gz_ci.sum() == 0:
        ...
        tmp = interpolate.griddata(
            marker_pos,
            np.nan_to_num(gaze_err, nan=np.nanmean(gaze_err)),
            (xg, yg),
            method="cubic",
            fill_value=np.nan,
        )
    if method == "griddata":
        ...
        gaze_err_image = tps.TPS.z(
            np.vstack([xg.flatten(), yg.flatten()]).T, to_fit, theta
        ).reshape(*xg.shape)
        if not extrapolate:
